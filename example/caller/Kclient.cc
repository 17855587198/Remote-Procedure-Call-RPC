...
void send_request(int thread_id, std::atomic<int> &success_count, std::atomic<int> &fail_count) {
    ...
    //对于SumToN函数需要进行模拟(同步方式发起rpc的调用请求，等待返回结果)
    Kuser::SumToNRequest sum_request;
    sum_request.set_n(100000);//求和
    Kuser::SumToNResponse sum_response;
    //controller.Reset();//复用,重置
    stub.SumToN(nullptr,&sum_request,&sum_response,nullptr);
        if (0 == sum_response.result().errcode()) {  // 检查响应中的错误码
            std::cout << "Sum result: " << sum_response.sum() << std::endl;  // 打印求和结果
            success_count++;  // 成功计数加 1
        } else {  // 如果响应中有错误
            std::cout << "RPC error: " << sum_response.result().errmsg() << std::endl;
            fail_count++;  // 失败计数加 1
        }
}
