...
// 全局的watcher观察器，用于接收ZooKeeper服务器的通知(即zkserver给zkclient的通知)
...
ZkClient::~ZkClient() {
    if (m_zhandle != nullptr) {
        zookeeper_close(m_zhandle);  // 关闭ZooKeeper连接，释放连接的资源
    }
}

// 启动ZooKeeper客户端，连接ZooKeeper服务器
/*
zookeeper_mt是多线程的版本,zookeeper的API程序提供了三个API线程,API调用线程以及网络I/O线程:
watcher回调线程
*/
void ZkClient::Start() {
    ...
    */
    ...
    m_zhandle = zookeeper_init(connstr.c_str(), global_watcher, 6000, nullptr, nullptr, 0);//此处的6000是会话的超时时间
